...
async def seed_admin():
    ...
    # Migração: se já existe um admin root legado (ex.: 'donas'), renomeia para o
    # ADMIN_USERNAME configurado e redefine a senha — mantém idempotência (roda 1x).
    legacy = await _db.admins.find_one({'role': 'root'})
    if legacy:
        await _db.admins.update_one(
            {'_id': legacy['_id']},
            {'$set': {'username': ADMIN_USERNAME, 'password_hash': pwd_hash, 'role': 'root'}},
        )
        logger.info(f"Migrated root admin to: {ADMIN_USERNAME}")
    else:
...
async def list_documents(skip: int = 0, limit: int = 500, q: str = '', tipo: str = '', user=Depends(require_admin)):
    """Lista cadastros que possuem documento de identificação anexado."""
    filt = {'form_data.docArquivoData': {'$exists': True, '$ne': ''}}
    if q:
        filt['$and'] = [{'$or': [
            {'nome': {'$regex': q, '$options': 'i'}},
            {'cpf': {'$regex': q}},
    ...
    if tipo:
        filt['form_data.docTipo'] = tipo
    proj = {
        '_id': 0,
        'cpf': 1, 'nome': 1, 'email': 1, 'created_at': 1, 'last_at': 1,
        'form_data.nome': 1, 'form_data.cpf': 1, 'form_data.email': 1,
        'form_data.docTipo': 1,
        'form_data.docArquivoNome': 1, 'form_data.docArquivoTipo': 1, 'form_data.docArquivoSize': 1,
        'form_data.docArquivoVersoNome': 1, 'form_data.docArquivoVersoTipo': 1, 'form_data.docArquivoVersoSize': 1,
        'form_data.docArquivoVersoData': 1,
    }
    cursor = _db.cadastros.find(filt, proj).sort('last_at', -1).skip(skip).limit(limit)
    items = []
    async for doc in cursor:
        fd = doc.get('form_data') or {}
        items.append({
            'cpf': doc.get('cpf') or fd.get('cpf'),
            'nome': fd.get('nome') or doc.get('nome'),
            'email': fd.get('email') or doc.get('email'),
            'doc_tipo': fd.get('docTipo'),
            'frente_nome': fd.get('docArquivoNome'),
            'frente_tipo': fd.get('docArquivoTipo'),
            'frente_size': fd.get('docArquivoSize'),
            'verso_nome': fd.get('docArquivoVersoNome'),
            'verso_tipo': fd.get('docArquivoVersoTipo'),
            'verso_size': fd.get('docArquivoVersoSize'),
            'has_verso': bool(fd.get('docArquivoVersoData')),
            'created_at': iso_utc(doc['created_at']) if isinstance(doc.get('created_at'), datetime) else doc.get('created_at'),
            'last_at': iso_utc(doc['last_at']) if isinstance(doc.get('last_at'), datetime) else doc.get('last_at'),
        })
...
async def export_documents_zip(payload: DocsExportIn, user=Depends(require_admin)):
    ...
    if payload.tipo:
        filt['form_data.docTipo'] = payload.tipo
    ...
    with zipfile.ZipFile(spool, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
        # batch_size limita memória ao processar cursor
        cursor = _db.cadastros.find(filt, {'_id': 0}).batch_size(20)
        async for doc in cursor:
            fd = doc.get('form_data') or {}
            cpf = doc.get('cpf') or fd.get('cpf') or 'sem-cpf'
            cpf_digits = re.sub(r'\D', '', cpf)
            nome = (fd.get('nome') or doc.get('nome') or 'candidato').strip()
            safe_nome = re.sub(r'[^A-Za-z0-9_\- ]', '_', nome)[:60].strip().replace(' ', '_')
            tipo = (fd.get('docTipo') or 'documento')
            folder = f"{cpf_digits}_{safe_nome}"
            had_any = False
            for side, dkey, nkey, tkey in [
                ('frente', 'docArquivoData', 'docArquivoNome', 'docArquivoTipo'),
                ('verso',  'docArquivoVersoData', 'docArquivoVersoNome', 'docArquivoVersoTipo'),
            ]:
                data_url = fd.get(dkey)
                if not data_url:
                    continue
                m = re.match(r'^data:[^;]+;base64,(.+)$', data_url)
                raw_b64 = m.group(1) if m else data_url
                try:
                    raw = base64.b64decode(raw_b64)
                except Exception:
                    continue
                file_nome = fd.get(nkey) or ''
                ext = ''
                if '.' in file_nome:
                    ext = '.' + file_nome.rsplit('.', 1)[1].lower()
                else:
                    tipo_mime = (fd.get(tkey) or '').lower()
                    if 'png' in tipo_mime: ext = '.png'
                    elif 'jpeg' in tipo_mime or 'jpg' in tipo_mime: ext = '.jpg'
                    elif 'pdf'  in tipo_mime: ext = '.pdf'
                fname = f"{folder}/{tipo}_{side}{ext}"
                zf.writestr(fname, raw)
                count_files += 1
                had_any = True
                # libera a referência ao buffer raw para o GC
                del raw
            if had_any:
                count_candidates += 1
            # libera o doc após processar
            del doc, fd
...
async def get_document_file(cpf: str, side: str, user=Depends(require_admin)):
    ...
    doc = await _db.cadastros.find_one({'cpf': cpf_digits}, {'_id': 0, 'form_data': 1, 'nome': 1, 'cpf': 1})
    if not doc:
        raise HTTPException(404, "Cadastro não encontrado")
    fd = doc.get('form_data') or {}
    if side == 'frente':
        data = fd.get('docArquivoData')
        nome = fd.get('docArquivoNome'); tipo = fd.get('docArquivoTipo'); size = fd.get('docArquivoSize')
    else:
        data = fd.get('docArquivoVersoData')
        nome = fd.get('docArquivoVersoNome'); tipo = fd.get('docArquivoVersoTipo'); size = fd.get('docArquivoVersoSize')
    if not data:
...
async def generate_pix_brcode(payload: Dict[str, Any]):
    ...
    nome = (s.get('pix_nome') or 'PREFEITURA ARAGUAINA').upper()
    cidade = (s.get('pix_cidade') or 'ARAGUAINA').upper()
...
def _build_telegram_message(insc: Dict[str, Any], settings: Dict[str, Any] = None) -> str:
    ...
    titulo = settings.get('telegram_titulo') or 'NOVA INSCRIÇÃO - ARAGUAÍNA'
